...
void SearchServer::AddDocument(int document_id, std::string_view document, DocumentStatus status, const std::vector<int>& ratings) {
    if (document_id < 0) {
        throw std::invalid_argument("Document with negative id"s);
    }
    if (documents_.count(document_id)) {
        throw std::invalid_argument("Document with the id of the previously added document"s);
    }
    if (!IsValidWord(document)) {
        throw std::invalid_argument("Presence of invalid characters"s);
    }

    auto words = SplitIntoWordsNoStop(document);
    const std::string document_str(document);
    documents_.emplace(document_id, DocumentData{SearchServer::ComputeAverageRating(ratings), status, document_str});
...
std::tuple<std::vector<std::string_view>, DocumentStatus> SearchServer::MatchDocument(const std::execution::parallel_policy& policy, std::string_view raw_query, int document_id) const {
    if ((document_id < 0) || (documents_.count(document_id) == 0)) {
        throw std::invalid_argument("Document id out of range"s);
    }

    const Query &query = ParseQuery(raw_query);
    const auto &word_freqs = document_to_word_freqs_.at(document_id);

    if (std::any_of(query.minus_words.begin(), query.minus_words.end(), [&word_freqs](const std::string_view word) {
